        ...
        if 'file' in request.FILES:
            ...
            try:
                ...
                if isinstance(uploaded_data, list):
                    data_list.extend(uploaded_data)
                    write_data_to_json(data_list)
                else:
                    return render(request, 'index.html', {'error': "Загруженный файл должен содержать список объектов.", 'data_list': data_list})
            except json.JSONDecodeError:
                return render(request, 'index.html', {'error': "Не удалось прочитать файл JSON.", 'data_list': data_list})
        else:
